    ...
    * { box-sizing: border-box; }
    ...
    .exam-timer {
      ...
      background:
        ...
        radial-gradient(circle at top left, rgba(86,214,227,0.16), transparent 34%);
      color: var(--ink);
      box-shadow: var(--shadow-soft);
      animation: rise-in 520ms ease-out both;
    }
    ...
    .timer-actions button {
      ...
      box-shadow: 0 8px 16px rgba(35, 79, 127, 0.06);
      transition: transform 160ms ease, border-color 160ms ease, box-shadow 160ms ease;
    }
    .timer-actions button:hover {
      transform: translateY(-1px);
      border-color: rgba(26, 174, 190, 0.26);
      box-shadow: 0 10px 22px rgba(35, 79, 127, 0.10);
    }
    .timer-status {
      grid-column: 1 / -1;
      min-height: 0.95rem;
    ...
    .score-summary {
      ...
      background:
        linear-gradient(180deg, rgba(255,255,255,0.98), rgba(247,251,254,0.98)),
        radial-gradient(circle at top right, rgba(86,214,227,0.14), transparent 34%);
      box-shadow: var(--shadow-soft);
    }
    .score-heading {
      display: flex;
    ...
    .score-stat.correct strong {
      color: var(--success);
    }
    .score-stat.incorrect strong {
      color: var(--danger);
    }
    .score-note {
      margin: 0;
...
<body>
  <main class="wrap">
    ...
    {% if is_mock_exam %}
    <section class="exam-timer" id="exam-timer" data-duration-seconds="10800" aria-label="Optional exam timer">
      <div class="timer-copy">
        <div class="timer-label">3 hr timer</div>
        <div class="timer-time" id="timer-display">03:00:00</div>
        <div class="timer-status" id="timer-status">Optional, saved on this device.</div>
      </div>
    ...
    {% if is_mock_exam %}
    <section class="score-summary" id="exam-score" data-total-cards="{{ flashcards|length }}" aria-label="Exam score summary">
      ...
      <div class="score-grid">
        ...
        <div class="score-stat incorrect">
          <strong id="score-incorrect">0</strong>
          <span>Incorrect</span>
        </div>
        <div class="score-stat">
          <strong id="score-answered">0 / {{ flashcards|length }}</strong>
          <span>Answered</span>
        </div>
        <div class="score-stat">
          <strong id="score-unanswered">{{ flashcards|length }}</strong>
          <span>Unanswered</span>
        </div>
      </div>
      <p class="score-note" id="score-note">Score is calculated as correct answers divided by total exam questions. Unanswered questions count as not correct.</p>
    </section>
    {% endif %}
  </main>
  <script>
    ...
    const scoreRoot = document.getElementById("exam-score");
    ...
    function updateCounts(state) {
      ...
      updateScoreSummary(mastered, review);
    }

    function updateScoreSummary(correct, incorrect) {
      if (!scoreRoot) return;
      const total = Number(scoreRoot.getAttribute("data-total-cards") || "0");
      const answered = correct + incorrect;
      const unanswered = Math.max(0, total - answered);
      const percent = total > 0 ? Math.round((correct / total) * 100) : 0;
      scorePercent.textContent = percent + "%";
      scoreCorrect.textContent = String(correct);
      scoreIncorrect.textContent = String(incorrect);
      scoreAnswered.textContent = answered + " / " + total;
      scoreUnanswered.textContent = String(unanswered);
      scoreNote.textContent = unanswered === 0
        ? "Exam complete. Score is correct answers divided by total exam questions."
        : "Score is calculated as correct answers divided by total exam questions. Unanswered questions count as not correct.";
    }
